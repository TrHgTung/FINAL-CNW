...
const Checkout = () => {
    ...
    useEffect(() => {
        ...
        //     const rescountry = await fetch("https://provinces.open-api.vn/api/?depth=1");
        //     const rescon = await rescountry.json();
        //     setCity(await rescon);
        // }
        // getLocation();
        fetch("https://provinces.open-api.vn/");
        ...
        axios.get("/api/?depth=1").then((res) => {
            if (res.status === 200) {
                setProvinces(Object.values(res.name));
            }
            setLoading(false);
        });
        axios.get("/api/?depth=1").then((res) => {
            if (res.status === 200) {
                if (res != null) {
                    setProvinceId(res.code);
                    setCityId(res.codename);
                }
                setFullName(
